...
\lstinputlisting[language=sh]{code/basic/runtc.sh}

\subsection{Runcpp.sh}
\lstinputlisting[language=sh]{code/basic/runcpp.sh}

\subsection{Others}
\lstinputlisting{code/basic/others.cpp}
...
\lstinputlisting{code/data-structure/lazy_propagation_seg.cpp}
...
\lstinputlisting{code/data-structure/linecontainer.cpp}
...
\subsection{Dinic}
\lstinputlisting{code/flow/Dinic2.cpp}

\subsection{PushRelabel}
\lstinputlisting{code/flow/PushRelabel.cpp}
...
\lstinputlisting{code/string/Suffix-Automaton.cpp}
...
\lstinputlisting{code/number-theory/LinearSieve.cpp}
...
\item 注意 n m 有沒有亂用
\item 1 不是質數
\item map TLE 用 gp\_hash\_table
\item 雙指針沒好好處理另一個指針
\item 取\%注意會不會有負數情況
\end{itemize}
